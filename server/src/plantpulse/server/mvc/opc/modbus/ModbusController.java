package plantpulse.server.mvc.opc.modbus;
...
public class ModbusController {
	...
	private static final Log log = LogFactory.getLog(ModbusController.class);

	private static final String OPC_TYPE = "MODBUS";
	...
	@RequestMapping(value = "/modbus/index", method = RequestMethod.GET)
	public String index(Model model, HttpServletRequest request) {
		model.addAttribute("opc_type", OPC_TYPE);
		return "modbus/index";
	}

	@RequestMapping(value = "/modbus/list")
	...
	@RequestMapping(value = "/modbus/add", method = RequestMethod.GET)
	...
	@RequestMapping(value = "/modbus/edit", method = RequestMethod.POST)
	...
	@RequestMapping(value = "/modbus/insert", method = RequestMethod.POST)
	...
	public @ResponseBody JSONResult insert(@ModelAttribute OPC opc, Model model, HttpServletRequest request) {
		try {
			...
			//
			return new JSONResult(JSONResult.SUCCESS, "모드버스 연결을 저장하였습니다.");
		} catch (Exception e) {
			log.error("Insert Exception : " + e.getMessage(), e);
			return new JSONResult(JSONResult.ERROR, "모드버스 연결을 저장할 수 없습니다. : 메세지 = [" + e.getMessage() + "]");
		}
	}

	@RequestMapping(value = "/modbus/update", method = RequestMethod.POST)
	@ResponseStatus(HttpStatus.OK)
	...
	@RequestMapping(value = "/modbus/delete/{opc_id}", method = RequestMethod.POST)
	@ResponseStatus(HttpStatus.OK)
	public @ResponseBody JSONResult delete(@PathVariable String opc_id, Model model, HttpServletRequest request) {
		try {
			...
			return new JSONResult(JSONResult.SUCCESS, "모드버스 연결을 삭제하였습니다.");
		} catch (Exception e) {
			log.error("Insert Exception : " + e.getMessage(), e);
			return new JSONResult(JSONResult.ERROR, "모드버스 연결을 삭제할 수 없습니다. : 메세지 = [" + e.getMessage() + "]");
		}
	}
