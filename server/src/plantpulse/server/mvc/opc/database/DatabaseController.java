package plantpulse.server.mvc.opc.database;
...
public class DatabaseController {
	...
	private static final String OPC_TYPE = "DATABASE";
	...
	@RequestMapping(value = "/database/index", method = RequestMethod.GET)
	...
	@RequestMapping(value = "/database/edit", method = RequestMethod.POST)
	...
	@RequestMapping(value = "/database/insert", method = RequestMethod.POST)
	...
	public @ResponseBody JSONResult insert(@ModelAttribute OPC opc, Model model, HttpServletRequest request) {
		try {
			...
			//
			return new JSONResult(JSONResult.SUCCESS, "데이터베이스 연결을 저장하였습니다.");
		} catch (Exception e) {
			log.error("Insert Exception : " + e.getMessage(), e);
			return new JSONResult(JSONResult.ERROR, "데이터베이스 연결을 저장할 수 없습니다. : 메세지 = [" + e.getMessage() + "]");
		}
	}
	...
	@RequestMapping(value = "/database/delete/{opc_id}", method = RequestMethod.POST)
	@ResponseStatus(HttpStatus.OK)
	public @ResponseBody JSONResult delete(@PathVariable String opc_id, Model model, HttpServletRequest request) {
		try {
			...
			return new JSONResult(JSONResult.SUCCESS, "데이터베이스 연결을 삭제하였습니다.");
		} catch (Exception e) {
			log.error("Insert Exception : " + e.getMessage(), e);
			return new JSONResult(JSONResult.ERROR, "데이터베이스 연결을 삭제할 수 없습니다. : 메세지 = [" + e.getMessage() + "]");
		}
	}
