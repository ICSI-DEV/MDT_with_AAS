package plantpulse.server.mvc.opc.file;
...
public class FileController {
	...
	private static final String OPC_TYPE = "FILE";
	...
	public @ResponseBody JSONResult insert(@ModelAttribute OPC opc, Model model, HttpServletRequest request) {
		try {
			...
			//
			return new JSONResult(JSONResult.SUCCESS, "파일/API 연결을 저장하였습니다.");
		} catch (Exception e) {
			log.error("Insert Exception : " + e.getMessage(), e);
			return new JSONResult(JSONResult.ERROR, "파일/API 연결을 저장할 수 없습니다. : 메세지 = [" + e.getMessage() + "]");
		}
	}
	...
	@RequestMapping(value = "/file/delete/{opc_id}", method = RequestMethod.POST)
	@ResponseStatus(HttpStatus.OK)
	public @ResponseBody JSONResult delete(@PathVariable String opc_id, Model model, HttpServletRequest request) {
		try {
			...
			//
			//
			return new JSONResult(JSONResult.SUCCESS, "파일/API 연결을 삭제하였습니다.");
		} catch (Exception e) {
			log.error("Insert Exception : " + e.getMessage(), e);
			return new JSONResult(JSONResult.ERROR, "파일/API 연결을 삭제할 수 없습니다. : 메세지 = [" + e.getMessage() + "]");
		}
	}
