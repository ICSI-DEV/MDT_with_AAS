package plantpulse.server.mvc.opc.plc;
...
public class PLCController {
	...
	private static final Log log = LogFactory.getLog(PLCController.class);

	private static final String OPC_TYPE = "PLC";
	...
	@RequestMapping(value = "/plc/index", method = RequestMethod.GET)
	public String index(Model model, HttpServletRequest request) {
		model.addAttribute("opc_type", OPC_TYPE);
		return "plc/index";
	}

	@RequestMapping(value = "/plc/list")
	...
	@RequestMapping(value = "/plc/add", method = RequestMethod.GET)
	...
	@RequestMapping(value = "/plc/edit", method = RequestMethod.POST)
	...
	@RequestMapping(value = "/plc/insert", method = RequestMethod.POST)
	...
	public @ResponseBody JSONResult insert(@ModelAttribute OPC opc, Model model, HttpServletRequest request) {
		try {
			...
			//
			return new JSONResult(JSONResult.SUCCESS, "PLC 연결을 저장하였습니다.");
		} catch (Exception e) {
			log.error("Insert Exception : " + e.getMessage(), e);
			return new JSONResult(JSONResult.ERROR, "PLC 연결을 저장할 수 없습니다. : 메세지 = [" + e.getMessage() + "]");
		}
	}

	@RequestMapping(value = "/plc/update", method = RequestMethod.POST)
	@ResponseStatus(HttpStatus.OK)
	...
	@RequestMapping(value = "/plc/delete/{opc_id}", method = RequestMethod.POST)
	@ResponseStatus(HttpStatus.OK)
	public @ResponseBody JSONResult delete(@PathVariable String opc_id, Model model, HttpServletRequest request) {
		try {
			...
			return new JSONResult(JSONResult.SUCCESS, "PLC 연결을 삭제하였습니다.");
		} catch (Exception e) {
			log.error("Insert Exception : " + e.getMessage(), e);
			return new JSONResult(JSONResult.ERROR, "PLC 연결을 삭제할 수 없습니다. : 메세지 = [" + e.getMessage() + "]");
		}
	}
