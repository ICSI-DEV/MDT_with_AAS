 ...
 *
 */
public class AssetPointDataJob extends SchedulingJob {

	private static final Log log = LogFactory.getLog(AssetPointDataJob.class);

	public AssetPointDataJob() {
		//
	}
	...
	public void run() {
		...
		try {
			...
			log.debug("Point asset data job starting...");
			...
			if (point_map != null) {
				...
				exec.execute(new Worker() {
						public String getName() { return "AssetPointDataJob"; };
						@Override
						public void execute() {
						    ...
						    log.debug("Asset point data all inserted : asset_size=[" + list.size() + "], process_time["  + end + "]ms");
						}
					});
			 };
				
		} catch (Exception e) {
			log.error("Asset point data job failed : " + e.getMessage(), e);
		}
	}
		...
		public void run() {
			...
			try {
				...
				if (tags != null && tags.size() > 0) {
					...
					// CASSANDRA 저장
					client.forInsert().insertAssetData(current_timestamp, model, tag_map, point_map);
					...
					// 
					log.debug("Asset data aync saved : asset_id=[" + model.getAsset_id() + "], asset_name=[" + model.getAsset_name() + "]");

				}
				; // tag size > 0

			} catch (Exception e) {
				log.error("Asset data aync save error : " + e.getMessage(), e);
			}
		}
