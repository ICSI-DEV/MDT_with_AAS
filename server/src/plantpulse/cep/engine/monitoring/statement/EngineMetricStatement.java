 ...
 *
 */
public class EngineMetricStatement extends Statement {

	private static final Log log = LogFactory.getLog(EngineMetricStatement.class);

	public EngineMetricStatement(String serviceId, String name) {
	...
	public String getEPL() {
		return "SELECT format_datetime(timestamp) as timestamp, inputCount, scheduleDepth, sum(inputCountDelta) as inputCountDelta FROM com.espertech.esper.client.metric.EngineMetric.win:time_batch(1 sec) WHERE engineURI='"
				+ serviceId + "' OUTPUT LAST ";
	}
		...
		try {
			...
			if (newEvents != null && newEvents.length > 0) {
				...
				if (event != null) {
					...
					PushClient client1 = ResultServiceManager.getPushService().getPushClient(MonitorConstants.PUSH_URL_ENGINE_STATUS + "/" + serviceId);
					client1.sendJSON(json);
				}
			}
		} catch (Exception ex) {
			log.error("EngineMetricStatement error : " + ex.getMessage(), ex);
		}
	}
