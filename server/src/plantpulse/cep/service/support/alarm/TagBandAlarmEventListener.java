...
public class TagBandAlarmEventListener implements UpdateListener {

	private static final Log log = LogFactory.getLog(TagBandAlarmEventListener.class);

	private AlarmConfig alarm_config;
	private Tag tag;

	public TagBandAlarmEventListener(AlarmConfig alarm_config, Tag tag) {
	...
	public void update(EventBean[] newEvents, EventBean[] arg1) {
		...
		try {
			...
			if (newEvents != null && newEvents.length > 0) {
				...
				if (event != null) {
					...
					if (TagUtils.isNumbericDataType(tag)) { //
						//메세지가 없는 경우
						if(StringUtils.isEmpty(alarm_config.getMessage())){
							String message = ConfigurationManager.getInstance().getApplication_properties().getProperty("tag.band.alarm.message",  AlarmDefaultMessage.DEFALUT_ALARM_BAND_MESSAGE);
							alarm_config.setMessage(message);
						};
						//
						alarm_config.setAlarm_config_priority(priority);
	
						//
						json.put("description", tag.getDescription());
						json.put("alias_name",  tag.getAlias_name());
						json.put("uinit",       tag.getUnit());
	
						//
						if(!json.containsKey("set_value")){
							String set_value = ""; // 설정된 임계 값
							if (band.equals("TRIP-LOW")) {
								set_value = tag.getTrip_lo();
							} else if (band.equals("LOW")) {
								set_value = tag.getLo();
							} else if (band.equals("LOW-LOW")) {
								set_value = tag.getLo_lo();
							} else if (band.equals("TRIP-HIGH")) {
								set_value = tag.getTrip_hi();
							} else if (band.equals("HIGH")) {
								set_value = tag.getHi();
							} else if (band.equals("HIGH-HIGH")) {
								set_value = tag.getHi_hi();
							}
							;
							json.put("set_value", set_value);
						};
					};
					...
					/// 2.불린형일 경우 핸들링
					///-----------------------------------------------------------------------------------
					if (TagUtils.isBooleanDataType(tag)) { //
							
						if(json.getString("value").equals("true")) {
							alarm_config.setMessage(tag.getBool_true_message());
						}
						if(json.getString("value").equals("false")) {
							alarm_config.setMessage(tag.getBool_false_message());
						};
						
						//메세지가 없는 경우
						if(StringUtils.isEmpty(alarm_config.getMessage())){
							String message = ConfigurationManager.getInstance().getApplication_properties().getProperty("tag.boolean.alarm.message", AlarmDefaultMessage.DEFAULT_ALARM_BOOLEAN_MESSAGE);
							alarm_config.setMessage(message);
						};
						//
						alarm_config.setAlarm_config_priority(priority);
						...
						//
						if(!json.containsKey("set_value")){
							String set_value =  json.getString("value");
							json.put("set_value", set_value);
						};
					};
					...
					if (!handler.aleadyAlarmed(alarm_config.getAlarm_config_id(), alarm_config.getInsert_user_id(), alarm_config.getAlarm_config_priority())) {
						...
						log.debug("__TAG_BAND_ALARM_JSON_DATA = " + data.toString());
			...
			log.error("Tag band alarm update listener failed : " + ex.getMessage(), ex);
		}
	}
